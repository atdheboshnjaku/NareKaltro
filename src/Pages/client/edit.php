...
if($objForm->isPost("name")) {
    ...
    if(!empty($objForm->getPost("email"))) 
    {
        ...
        $existingUser = $objUser->getUserByEmail($email, $client['id']);
...
<div class="profile-box-ctn">
    ...
    <form action="" method="post" class="add-form">
        <div class="profile-card">
            ...
            <p>
                <span>Phone</span>
                <input type="number" name="number" value="<?php echo $objForm->stickyText('number', $client['number']); ?>" placeholder="Client number" autocomplete="off">
            </p>
        ...
        <div class="profile-info">
                    ...
                    <optgroup label="User location">
                        <?php foreach($locations as $location) { ?>
                            <option value="<?php echo $location['id']; ?>"
                                <?php echo $objForm->stickySelect('location_id', $location['id'], $client['location_id']); ?>>
            ...
            <p>
                <span>Email</span>
                <input type="email" name="email" value="<?php echo $objForm->stickyText('email', $client['email']); ?>" placeholder="" autocomplete="false">
            </p>
            <?php echo $objValidation->validate('name'); ?>
            <p>
                <span>Client name</span>
                <input type="hidden" name="status" value="1">
                <input type="text" name="name" value="<?php echo $objForm->stickyText('name', $client['name']); ?>" placeholder="" >
            </p>
            <p>
                <input type="submit" name="submit" class="blue-btn alab" value="Update client">
            </p>
        </div>
...
<script>
    ...
    $('select[name="country"]').on('change',function() {
        ...
        if (country_id) {
            ...
            success: function(data) {
                ...
                if (page_load == true) {
                    $('#state').val(<?php echo $client['state']; ?>).trigger('change'); //assign slected value after element option is added in dom
    ...
    $('#country').val(<?php echo $client['country']; ?>).trigger('change');
    ...
    $('select[name="state"]').on('change',function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $('#city').val(<?php echo $client['city']; ?>).trigger('change');
            }
        });
        }else {
             $('select[name="city"]').empty();
